class Field {
  ...
  setMatrixGravityCircle(centerPoint, radius, gravity) {
    ...
    for(let theta = 0;  theta < 2 * Math.PI; theta += step) {
      ...
      const x = Math.ceil(h + gx - 0.10899);
      const y = Math.ceil(k - gy - 0.10899);
  ...
  drawGravitySize(px, py, gravitySize, space) {
    ...
    if (this.drawVector) {
      ...
      const xStrength = this.xMatrix[px][py];
      const yStrength = this.yMatrix[px][py];
      const dx = Math.sqrt(Math.abs(xStrength) * 500) * Math.sign(xStrength);
      const dy = Math.sqrt(Math.abs(yStrength) * 500) * Math.sign(yStrength);

      const averageStrength = (Math.abs(dx) + Math.abs(dy)) / 2; // TODO: un-simplify
      const color = Math.min( Math.ceil(averageStrength) * 3, 50 );

      space.drawLine(
        xCenter, yCenter,
        xCenter + dx, yCenter + dy,
